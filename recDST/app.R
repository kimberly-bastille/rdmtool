    ...
    sidebarLayout(
        sidebarPanel(
          shinyWidgets::awesomeCheckboxGroup(
            ...
            inline = TRUE,
            status = "danger"),
          sliderInput(inputId = "NJ_SFsm_baglimit",
                      label ="Summer Flounder Small Bag Limit NJ",
...
server <- function(input, output) {
  ...
  observeEvent(input$runmeplease, {
    output$tableout<- renderTable({
      state <- input$state
      NJ_SFsm_baglimit <- input$NJ_SFsm_baglimit
      ...
      directed_trips_table<-data.frame(readr::read_csv(file.path(here::here("data-raw/directed trips and regulations 2020.csv")))) %>% 
        dplyr::mutate(fluke_bag1= dplyr::case_when(state == "NJ" ~ c(NJ_SFsm_baglimit)), 
