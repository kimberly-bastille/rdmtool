...
/*
egen pctile_x=pctile(tot_ntrip_not_trunc) , p(15) by(group)
gen tab2=1 if tot_ntrip_not_trunc>0 & tot_ntrip_not_trunc>pctile_x
egen sumtab2=sum(tab2), by(group)
...
*when # of trips that catch zero fish =0 it creates problems. Replace these with the mean across draws 
...
*need to make sure that for each draw, total keep + total release = total catch
order domai*
*drop if disp=="catch"
...
/*
*for aggregate 
drop domain    species disp
rename disp4 domain
rename domain xvar
split xvar, parse("_")
drop xvar1
rename xvar2 species
rename xvar3 disp
rename xvar domain

gen domain2=species
order domain*   species disp
*/
...
*gen domain2=month+"_"+mode+"_"+species
...
*merge 1:1 nfish month mode species using `keep'
...
*merge 1:1 nfish  species using `keep'


*merge 1:1 nfish month mode species using `catch'
*drop _merge


*a) compute the total number of trips based on the keep distirbution 
forv i =1/150{
...
*egen check =sum(tot_ntrip2), by(domain disp)
*order check
*drop check 
gen domain = month+"_"+mode+"_"+species+"_"+disp
order domain month mode species disp nfish
*egen check =sum(tot_ntrip2), by(domain2)
*order check

*gen domain = species+"_"+disp
*order domain species disp nfish
...
egen rowmin=rowmin(tot_ntrip1-tot_ntrip150)
drop rowmin
/*
forv i =1/150{
