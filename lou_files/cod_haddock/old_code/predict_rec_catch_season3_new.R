...
reg_select_list=c( "SQ", "cod_min_minus1" , "cod_min_plus1", "cod_min_minus2", "cod_min_plus2", "cod_bag_minus1", "cod_bag_plus1", 
                   "cod_bag_plus2", "cod_bag_plus4", "cod_open_aug", "cod_min_minus3",  "cod_min_minus5", "cod_bag_plus2_min_minus2", 
                   "cod_bag_plus4_min_minus2", "cod_open_aug_bag_plus1", "cod_open_aug_min_minus1", "cod_open_aug_min_minus2", 
                   "hadd_bag_minus1", "hadd_bag_plus1", "hadd_bag_minus3","hadd_bag_plus3", "hadd_bag_minus5", "hadd_bag_plus5", 
                   "hadd_bag_minus9", "hadd_bag_plus9", "hadd_min_minus1", "hadd_min_plus1", "hadd_min_minus2", "hadd_min_plus2")


reg_select_list=c(  "cod_min_minus1" , "cod_min_plus1", "cod_min_minus2", "cod_min_plus2", "cod_bag_minus1", "cod_bag_plus1", 
                   "cod_bag_plus2", "cod_bag_plus4", "cod_open_aug", "cod_min_minus3",  "cod_min_minus5", "cod_bag_plus2_min_minus2", 
                   "cod_bag_plus4_min_minus2", "cod_open_aug_bag_plus1", "cod_open_aug_min_minus1", "cod_open_aug_min_minus2", 
                   "hadd_bag_minus1", "hadd_bag_plus1", "hadd_bag_minus3","hadd_bag_plus3", "hadd_bag_minus5", "hadd_bag_plus5", 
                   "hadd_bag_minus9", "hadd_bag_plus9", "hadd_min_minus1", "hadd_min_plus1", "hadd_min_minus2", "hadd_min_plus2")
...
for (reg_select in reg_select_list){
...
for(i in unique(baseline_comparison1$mrip_index)){
  ...
  if(reg_select=="cod_min_minus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_min_y2!=254~cod_min_y2-2.54, TRUE ~ cod_min_y2))
  }

  if(reg_select=="cod_min_plus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_min_y2!=254~cod_min_y2+2.54,TRUE~cod_min_y2))
  }

  if(reg_select=="cod_min_minus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_min_y2!=254~cod_min_y2-5.04, TRUE ~ cod_min_y2))
  }

  if(reg_select=="cod_min_minus3"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_min_y2!=254~cod_min_y2-7.62, TRUE ~ cod_min_y2))
  }

  if(reg_select=="cod_min_minus5"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_min_y2!=254~cod_min_y2-12.7, TRUE ~ cod_min_y2))
  }

  if(reg_select=="cod_min_plus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_min_y2!=254~cod_min_y2+5.04,TRUE~cod_min_y2))
  }

  if(reg_select=="cod_bag_minus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2 =case_when(cod_bag_y2!=0~ cod_bag_y2 -1, TRUE ~ cod_bag_y2))

    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_bag_y2==0~254, TRUE ~ cod_min_y2))
  }

  if(reg_select=="cod_bag_plus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2 =case_when(cod_bag_y2!=0~ cod_bag_y2 +1, TRUE ~ cod_bag_y2))
  }

  if(reg_select=="cod_bag_plus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2 =case_when(cod_bag_y2!=0~ cod_bag_y2 +2, TRUE ~ cod_bag_y2))
  }

  if(reg_select=="cod_bag_plus2_min_minus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2 =case_when(cod_bag_y2!=0~ cod_bag_y2 +2, TRUE ~ cod_bag_y2))
  ...
  if(reg_select=="cod_bag_plus4_min_minus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2 =case_when(cod_bag_y2!=0~ cod_bag_y2 +4, TRUE ~ cod_bag_y2))
  ...
  if(reg_select=="cod_bag_plus4"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2 =case_when(cod_bag_y2!=0~ cod_bag_y2 +4, TRUE ~ cod_bag_y2))
  }


  if(reg_select=="hadd_bag_minus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 -1, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_bag_plus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 +1, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_bag_minus3"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 -3, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_bag_plus3"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 +3, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_bag_minus5"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 -5, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_bag_plus5"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 +5, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_bag_minus9"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 -9, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_bag_plus9"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_bag_y2 =case_when(hadd_bag_y2!=0~ hadd_bag_y2 +9, TRUE ~ hadd_bag_y2))
  }

  if(reg_select=="hadd_min_minus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_min_y2=case_when(hadd_min_y2!=254~hadd_min_y2-2.54, TRUE ~ hadd_min_y2))
  }

  if(reg_select=="hadd_min_plus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_min_y2=case_when(hadd_min_y2!=254~hadd_min_y2+2.54,TRUE~hadd_min_y2))
  }

  if(reg_select=="hadd_min_minus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_min_y2=case_when(hadd_min_y2!=254~hadd_min_y2-5.04, TRUE ~ hadd_min_y2))
  }

  if(reg_select=="hadd_min_plus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(hadd_min_y2=case_when(hadd_min_y2!=254~hadd_min_y2+5.04,TRUE~hadd_min_y2))
  }


  if(reg_select=="cod_open_aug"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2=case_when(month1==8~1,TRUE~cod_bag_y2))

    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(month1==8~58.42,TRUE~cod_min_y2))
  }

  if(reg_select=="cod_open_aug_min_minus1"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2=case_when(month1==8~1,TRUE~cod_bag_y2))
  ...
  if(reg_select=="cod_open_aug_min_minus2"){
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_bag_y2=case_when(month1==8~1,TRUE~cod_bag_y2))
    ...
    directed_trips_table<-directed_trips_table %>%
      dplyr::mutate(cod_min_y2=case_when(cod_min_y2!=254~cod_min_y2-5.08, TRUE ~ cod_min_y2))
  }
  ...
  if(cod_catch_check !=0){
    ...
    if (cod_release_2_keep==1 & sum_cod_rel>0){
      ...
      # the number of illegal harvest is the same as the baseline year 
      ...
      # the number of illegal harvest is proportionate to the baseline year. 
    ...
    #sum(catch_size_data$keep)+sum(catch_size_data$release)
    }
    ...
    if (cod_keep_2_release==1 & sum_cod_keep>0){
      ...
      if (all_cod_keep_2_release==0){
        ...
        #v5
        # If there are fewer fish released (more kept) under the new regulations/length distribution compared to old, 
        ...
        # the number of voluntary release is proportionate to base year 
  ...
  if (had_catch_check!=0){
    ...
    if (hadd_release_2_keep==1 & sum_hadd_rel>0){
      ...
      # Number of illegal harvests:
      #n_sub_had_kept=min(n_sub_had_kept_base, round(h_star_hadd_release_to_keep_variable*n_row_had_re_allocate)) 
      ...
      catch_size_data_had_re_allocate <- catch_size_data_had_re_allocate %>%
        ...
        #dplyr::mutate(cutoff = case_when(diff_bag<=0~5, TRUE~max(1, 5-diff_bag)))
    ...
    if (hadd_keep_2_release==1 & sum_hadd_kept>0){
      ...
      if (all_hadd_keep_2_release==0){
        ...
        #n_legal_hadd_rel=min(n_legal_hadd_rel_base, round(h_star_hadd_keep_to_release_variable*n_row_had_re_allocate))
...
#write_xlsx(output2, paste0(output_data_cd, reg_select, "_v2.xlsx"))
#write_xlsx(output2, paste0(output_data_cd, reg_select, "_v3.xlsx"))
write_xlsx(output2, paste0(output_data_cd, reg_select, "_v4_month.xlsx"))
