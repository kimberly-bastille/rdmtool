...
gen my_dom_id_string=area_s+"_"+mode1+"_"+common_dom+"_"+month
...
replace my_dom_id_string=mode1+"_"+common_dom+"_"+month
...
keep mode month var b ll ul 

reshape wide b ll ul, i(mode month) j(var) string
...
set obs 5
gen mode="sh"
gen month="04" if _n==1
replace month="05" if _n==2
replace month="06" if _n==3
replace month="08" if _n==4
...
export delimited using "$draw_file_cd\MRIP_catch_totals_month.csv", replace 
