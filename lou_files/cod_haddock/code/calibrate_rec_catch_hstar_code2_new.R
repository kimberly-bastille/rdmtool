...
if(cod_catch_check !=0){
  ...
  if (cod_release_2_keep==1 & sum_cod_rel>0){
    ...
    sum_rel_cod_re_allocate=sum(catch_size_data_cod_re_allocate$release)
...
if (had_catch_check!=0){
  ...
  sum_rel_hadd_re_allocate=sum(catch_size_data_had_re_allocate$release)
  ...
  n_sub_hadd_kept=round(h_star_hadd_release_to_keep_variable*n_row_had_re_allocate)
  
  catch_size_data_had_re_allocate<-catch_size_data_had_re_allocate %>% 
    dplyr::mutate(keep_new=dplyr::case_when(fishid2<=n_sub_hadd_kept~1, TRUE~ 0))
