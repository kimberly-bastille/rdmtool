...
twoway (rcap llcod_catch_mrip ulcod_catch_mrip wave_2023 if year=="2023" & mode=="pr",  vertical lcolor(blue)) ///
       ...
       ytitle("Cod catch-per-trip, # fish", size(medium)) ///
       title("Cod catch-per-trip estimates by wave, private sector", size(medium))
graph export "$figure_cd/cod_catch_trip_pr_23_24.png", as(png) replace
...
twoway (rcap llcod_keep_mrip ulcod_keep_mrip wave_2023 if year=="2023" & mode=="pr",  vertical lcolor(blue)) ///
       ...
       legend(order(2 "2023 Estimates" ///
                    4 "2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("wave", size(medium)) ///
       ytitle("Cod harvest (a+b1)-per-trip, # fish", size(medium)) ///
       title("Cod harvest-per-trip estimates by wave, private sector", size(medium))
graph export "$figure_cd/cod_harvest_trip_pr_23_24.png", as(png) replace
...
twoway (rcap llcod_catch_mrip ulcod_catch_mrip wave_2023 if year=="2023" & mode=="fh",  vertical lcolor(blue)) ///
       ...
       xtitle("wave", size(medium)) ///
       ytitle("Cod catch-per-trip, # fish", size(medium)) ///
       title("Cod catch-per-trip estimates by wave, for-hire sector", size(medium))
graph export "$figure_cd/cod_catch_trip_fh_23_24.png", as(png) replace
...
twoway (rcap llcod_keep_mrip ulcod_keep_mrip wave_2023 if year=="2023" & mode=="fh",  vertical lcolor(blue)) ///
       ...
       xtitle("wave", size(medium)) ///
       ytitle("Cod harvest (a+b1)-per-trip, # fish", size(medium)) ///
       title("Cod harvest-per-trip estimates by wave, for-hire sector", size(medium))
graph export "$figure_cd/cod_harvest_trip_fh_23_24.png", as(png) replace
...
twoway (rcap llhadd_catch_mrip ulhadd_catch_mrip wave_2023 if year=="2023" & mode=="pr",  vertical lcolor(blue)) ///
       ...
       legend(order(2 "2023 Estimates" ///
                    4 "2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("wave", size(medium)) ///
       ytitle("Haddock catch-per-trip, # fish", size(medium)) ///
       title("Haddock catch-per-trip estimates by wave, private sector", size(medium))
graph export "$figure_cd/hadd_catch_trip_pr_23_24.png", as(png) replace
...
twoway (rcap llhadd_keep_mrip ulhadd_keep_mrip wave_2023 if year=="2023" & mode=="pr",  vertical lcolor(blue)) ///
       ...
       legend(order(2 "2023 Estimates" ///
                    4 "2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("wave", size(medium)) ///
       ytitle("Haddock harvest (a+b1)-per-trip, # fish", size(medium)) ///
       title("Haddock harvest-per-trip estimates by wave, private sector", size(medium))
graph export "$figure_cd/hadd_keep_trip_pr_23_24.png", as(png) replace
...
twoway (rcap llhadd_catch_mrip ulhadd_catch_mrip wave_2023 if year=="2023" & mode=="fh",  vertical lcolor(blue)) ///
       ...
       xtitle("wave", size(medium)) ///
       ytitle("Haddock catch-per-trip, # fish", size(medium)) ///
       title("Haddock catch-per-trip estimates by wave, for-hire sector", size(medium))
graph export "$figure_cd/hadd_catch_trip_fh_23_24.png", as(png) replace
...
twoway (rcap llhadd_keep_mrip ulhadd_keep_mrip wave_2023 if year=="2023" & mode=="fh",  vertical lcolor(blue)) ///
       ...
       xtitle("wave", size(medium)) ///
       ytitle("Haddock harvest (a+b1)-per-trip, # fish", size(medium)) ///
       title("Haddock harvest-per-trip estimates by wave, for-hire sector", size(medium))   
graph export "$figure_cd/hadd_keep_trip_fh_23_24.png", as(png) replace
...
drop if FY=="."
drop my_dom_id_string my_dom_id_string1 my_dom_id_string3 
keep  wave FY var b ll ul se

reshape wide b se ll ul, i( wave FY) j(var) string
renvarlab b*, predrop(1)
ds  wave FY, not
renvarlab `r(varlist)', postfix(_mrip)
sort  FY wave 

ds  wave FY se* ll* ul*, not
mvencode `r(varlist)', mv(0) over
sort  FY wave
encode FY, gen(yyr2)
encode wave, gen(wv2)
...
twoway (rcap llcod_catch_mrip ulcod_catch_mrip wave_2023 if FY=="2023" ,  vertical lcolor(blue)) ///
       ...
       legend(order(2 "FY 2023 Estimates" ///
                    4 "FY 2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("wave", size(medium)) ///
       ytitle("Cod catch-per-trip, # fish", size(medium)) ///
       title("Cod catch-per-trip estimates by wave, all fishing modes", size(medium))
graph export "$figure_cd/cod_catch_trip_all_23_24.png", as(png) replace
...
twoway (rcap llcod_keep_mrip ulcod_keep_mrip wave_2023 if year=="2023" ,  vertical lcolor(blue)) ///
       ...
       xtitle("wave", size(medium)) ///
       ytitle("Cod harvest (a+b1)-per-trip, # fish", size(medium)) ///
       title("Cod harvest-per-trip estimates by wave,all fishing modes", size(medium))
graph export "$figure_cd/cod_harvest_trip_all_23_24.png", as(png) replace
	   

	   
twoway (rcap llhadd_catch_mrip ulhadd_catch_mrip wave_2023 if FY=="2023" ,  vertical lcolor(blue)) ///
       (scatter hadd_catch_mrip wave_2023 if FY=="2023" ,   msymbol(circle) msize(vsmall) mcolor(blue)) ///
       (rcap  llhadd_catch_mrip ulhadd_catch_mrip wave_2024 if FY=="2024" , vertical   lcolor(red)) ///
       (scatter hadd_catch_mrip wave_2024 if FY=="2024" , ylabel(#15,labsize(vsmall) angle(horizontal) ) msymbol(circle) msize(vsmall) mcolor(red)), ///
       legend(order(2 "FY 2023 Estimates" ///
                    4 "FY 2024 Estimates") region(lcolor(none)) position(1) ring(0) rows(2) size(small)) ///
       xtitle("wave", size(medium)) ///
       ytitle("Haddock catch-per-trip, # fish", size(medium)) ///
       title("Haddock catch-per-trip estimates by wave, all fishing modes", size(medium))
graph export "$figure_cd/hadd_catch_trip_all_23_24.png", as(png) replace
...
twoway (rcap llhadd_keep_mrip ulhadd_keep_mrip wave_2023 if year=="2023" ,  vertical lcolor(blue)) ///
       ...
       xtitle("wave", size(medium)) ///
       ytitle("Haddock harvest (a+b1)-per-trip, # fish", size(medium)) ///
       title("Haddock harvest-per-trip  estimates by wave,all fishing modes", size(medium))   
graph export "$figure_cd/hadd_keep_trip_all_23_24.png", as(png) replace
...
gen my_dom_id_string=common_dom+"_"+area_s+"_"+fyr2
replace my_dom_id_string=subinstr(ltrim(rtrim(my_dom_id_string))," ","",.)
encode my_dom_id_string, gen(my_dom_id)
...
split my_dom, parse(_)
rename my_dom_id_string3 FY
drop if FY=="."

drop my_dom_id_string my_dom_id_string1 my_dom_id_string2 
keep  FY var b ll ul se

reshape wide b se ll ul, i( FY) j(var) string
renvarlab b*, predrop(1)
ds   FY, not
renvarlab `r(varlist)', postfix(_mrip)
sort  FY  
...
set obs 4
gen FY=2021 if _n==1
replace FY=2022 if _n==2
replace FY=2025 if _n==3
replace FY=2026 if _n==4
tempfile yrs
save `yrs', replace 
restore
...
gen year_2023 = FY - 0.1 if year == 2023
gen year_2024 = FY + 0.1 if year == 2024

twoway (rcap llcod_catch_mrip ulcod_catch_mrip FY  if FY<=2023,  vertical lcolor(blue)) ///
       (scatter cod_catch_mrip FY  if FY<=2023,   msymbol(circle) msize(vsmall) mcolor(blue)) ///
       (rcap  llcod_catch_mrip ulcod_catch_mrip FY  if FY>2023, vertical   lcolor(red)) ///
       (scatter cod_catch_mrip FY  if FY>2023, ylabel(#15,labsize(vsmall) angle(horizontal) ) msymbol(circle) msize(vsmall) mcolor(red)), ///
       legend(order(2 "FY 2023 Estimates" ///
                    4 "FY 2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("", size(medium)) xlab(2023 2024) ///
       ytitle("Cod catch-per-trip, # fish", size(medium)) ///
       title("Cod catch-per-trip estimates by fishing year, all fishing modes", size(medium))
graph export "$figure_cd/cod_catch_trip_all_annual_23_24.png", as(png) replace
...
twoway (rcap llcod_keep_mrip ulcod_keep_mrip year  if year<=2023,  vertical lcolor(blue)) ///
       ...
       legend(order(2 "2023 Estimates" ///
                    4 "2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("", size(medium)) xlab(2023 2024) ///
       ytitle("Cod harvest-per-trip, # fish", size(medium)) ///
       title("Cod harvest-per-trip estimates by year (exluding wave 6), all fishing modes", size(medium))
graph export "$figure_cd/cod_harvest_trip_all_annual_23_24.png", as(png) replace
	   

	   
twoway (rcap llhadd_catch_mrip ulhadd_catch_mrip FY  if FY<=2023,  vertical lcolor(blue)) ///
       (scatter hadd_catch_mrip FY  if FY<=2023,   msymbol(circle) msize(vsmall) mcolor(blue)) ///
       (rcap  llhadd_catch_mrip ulhadd_catch_mrip FY  if FY>2023, vertical   lcolor(red)) ///
       (scatter hadd_catch_mrip FY  if FY>2023, ylabel(#15,labsize(vsmall) angle(horizontal) ) msymbol(circle) msize(vsmall) mcolor(red)), ///
       legend(order(2 "FY 2023 Estimates" ///
                    4 "FY 2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("", size(medium)) xlab(2023 2024) ///
       ytitle("Haddock catch-per-trip, # fish", size(medium)) ///
       title("Haddock catch-per-trip estimates by fishing year, all fishing modes", size(medium))
graph export "$figure_cd/hadd_catch_trip_all_annual_23_24.png", as(png) replace
...
twoway (rcap llhadd_keep_mrip ulhadd_keep_mrip year  if year<=2023,  vertical lcolor(blue)) ///
       ...
       legend(order(2 "2023 Estimates" ///
                    4 "2024 Estimates") region(lcolor(none)) position(11) ring(0) rows(2) size(small)) ///
       xtitle("", size(medium)) xlab(2023 2024) ///
       ytitle("Haddock harvest-per-trip, # fish", size(medium)) ///
       title("Haddock harvest-per-trip estimates by year (exluding wave 6), all fishing modes", size(medium))
graph export "$figure_cd/hadd_harvest_trip_all_annual_23_24.png", as(png) replace
