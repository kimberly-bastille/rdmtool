...
twoway	(scatter prob length if year==2024, connect(direct) lcol(black)   lwidth(medthick)  lpat(solid) msymbol(i)  ///
			title("Cod Length structure 2024", size(small)) xline(23, lcol(gray)) ///
			ytitle("proportion fish-at-length", size(small)) ytick(, angle(horizontal) labsize(small)) xtitle(Length inches, size(small)) xlab(#20, labsize(small)) ///
...
graph export "$figure_cd/cod_prop_NaL_24.png", as(png) replace

su prob if year==2025 & length>=23
local sum25=round(`r(sum)'*100, .01)
			
twoway	(scatter prob length if year==2025, connect(direct) lcol(black)   lwidth(medthick)  lpat(solid) msymbol(i)  ///
			title("Cod Length structure 2025", size(small)) xline(23, lcol(gray)) ///
			ytitle("proportion fish-at-length", size(small)) ytick(, angle(horizontal) labsize(small)) xtitle(Length inches, size(small)) xlab(#20, labsize(small)) ///
			ylab(, labsize(small) angle(horizontal)) xtick(#20, labsize(small))  lpat(solid) msymbol(i) ///
			note("% fish equal to or longer than 23 inches: `sum25'%" )) 		
graph export "$figure_cd/cod_prop_NaL_25.png", as(png) replace
...
graph export "$figure_cd/hadd_prop_NaL_24.png", as(png) replace
...
graph export "$figure_cd/hadd_prop_NaL_25.png", as(png) replace
