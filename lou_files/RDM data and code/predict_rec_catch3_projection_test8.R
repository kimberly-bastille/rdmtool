...
for (s in statez){
  ...
  NJ_bias_CV_trips<- readr::read_csv(file.path(here::here("differences_CV_trips_NJ.csv")),  show_col_types = FALSE) 
  
  NJ_bias_catch<- readr::read_csv(file.path(here::here("differences_total_catch_NJ.csv")),  show_col_types = FALSE) 
  ...
  directed_trips<- readRDS(here::here(paste0("directed_trips_2024/directed_trips_",state1,"_coastwide3.rds"))) %>% 
    dplyr::select(-X)
  
  sf_size_dat <- readr::read_csv(file.path(here::here(paste0("size_data/fluke_prob_star_2024_", state1,".csv"))),  show_col_types = FALSE) #%>%
  bsb_size_dat <- readr::read_csv(file.path(here::here(paste0("size_data/bsb_prob_star_2022_", state1, ".csv"))),  show_col_types = FALSE) #%>%
  scup_size_dat <- readr::read_csv(file.path(here::here(paste0("size_data/scup_prob_star_2024_", state1, ".csv"))),  show_col_types = FALSE)# %>%
  ...
  for (d in 1:100){
    ...
    # l_w_conversion[is.na(l_w_conversion)] <- 0
    f_dowle3(l_w_conversion)
    
    
    #For Kim: might need to change where you import of calibration_output_by_period in the next line. I don't think we 
    #actually need to import costs_new_all files here. We will need to import costs_new_all down at the utility function part, 
    #in order to get utility parameters and trip costs
    
    #calibration_output_by_period<- readRDS(here::here(paste0("calibration/pds_NJ_",x,"_test1.rds"))) #%>% 
    calibration_output_by_period<- readRDS(here::here(paste0("calibration/pds_", state1,"_",x,"_test1.rds"))) #%>% 
    
    #costs_new_all<- readRDS(here::here(paste0("calibration/costs_NJ_",x,"_test1.rds"))) #%>% 
    costs_new_all<- readRDS(here::here(paste0("calibration/costs_", state1, "_",x,"_test1.rds"))) #%>% 
    ...
    calendar_2024_adjust1 <- calendar_2024_adjust %>% dplyr::filter(draw==d) 
    
    MRIP_harvest_weights1<-MRIP_harvest_weights %>% dplyr::filter(draw==d)
    
    NJ_bias_CV_trips1<- NJ_bias_CV_trips %>% dplyr::filter(draw==d)
    
    NJ_bias_catch1<- NJ_bias_catch %>% dplyr::filter(draw==d) %>% 
    ...
    sf_catch_data<- readr::read_csv(paste0("C:/Users/andrew.carr-harris/Desktop/HCR catch data 2022/", state1, " catch draws 2024 draw4 ", x, ".csv"),  show_col_types = FALSE)
    ...
    sf_catch_data <- sf_catch_data %>% 
      ...
      dplyr::select(c(catch_draw, tripid, state, tot_cat_sf, tot_cat_bsb, tot_cat_scup, mode1, period2)) %>% 
    ...
    mean_trip_data <- mean_trip_data %>% ## ADD mean_kr_total for each species by tripid and period2
      data.table::as.data.table() #%>%
    
    # mean_trip_data[is.na(mean_trip_data)] <- 0
    f_dowle3(mean_trip_data)
    ...
    sims <- calibration_data %>%
      ...
      dplyr::left_join(calendar_2024_adjust1, by=c("state", "month", "mode1")) %>% 
    ...
    predictions_test[[d]]<- rbind(prediction_mode, prediction_all_mode, l_w_mode, l_w_sum) %>% 
  ...
  predictions_full= list.stack(predictions_test, fill=TRUE)
  write_xlsx(predictions_full,paste0("coastwide3_projections_11_20_", state1, ".xlsx")) 
  
  
  
  state_output[[s]]<-predictions_full
...
state_output_full= list.stack(state_output, fill=TRUE)
#write_xlsx(state_output_full,"projections1.xlsx") #first status-quo projections, NJ, RI, DE
#write_xlsx(state_output_full,"projections2.xlsx") #first status-quo projections, MA, CT, VA, NC
#write_xlsx(state_output_full,"projections3.xlsx") #first status-quo projections, NY, MD
#write_xlsx(state_output_full,"projections4.xlsx") #new status-quo projections, NYws 1-50
#write_xlsx(state_output_full,"projections_11_9.xlsx")
