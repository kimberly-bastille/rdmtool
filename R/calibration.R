...
calibrate_rec_catch <- function(state1,
  ...
  sf_catch_data<- subset(sf_catch_data, select=c(fishid, tripid, keep_tot, release, period2, catch_draw, mode, month)) %>%
  ...
  period_vec1 <- period_vec %>%
    dplyr::mutate(beta_sqrt_sf_keep= rnorm(nrow(period_vec), mean = .8093247, sd = 1.288768),
                  ...
                  beta_sqrt_sf_bsb_keep = rnorm(nrow(period_vec), mean=substitution_mean_parameter  , sd = substitution_sd_parameter ),
