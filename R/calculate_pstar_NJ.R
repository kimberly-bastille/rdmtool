...
calculate_pstar_NJ <- function(m){ # m = mode
  ...
  sf_catch_data <- read.csv(file.path(here::here("data-raw/catch/NJ catch draws 2022 draw 5.csv"))) %>% 
  ...
  trip_costs<-data.frame(read.csv(file.path(here::here("data-raw/trip_costs_state_summary.csv"))))
  ...
  period_names<-trip_data %>%  dplyr::select(period, period2)
